...
describe("HTTP redirect", function() {
  ...
  it("redirects with 302 by default", function(done) {
    request(app).get('/foo').expect(302).expect("Location", "/baz").end(done);
  });

  it("redirects with the given status code", function(done) {
    request(app).get('/bar').expect(301).expect("Location", "/baz").end(done);
  });

  it("returns empty body", function(done) {
    request(app).get("/foo").expect("").expect("Content-Length", 0).end(done);
  });
});
