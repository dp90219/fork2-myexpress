...
describe('Add handlers to a route', function() {
  ...
  it('pushed action object to the stack', function() {
...
describe('Implement Route Handlers Invokation:', function() {
  ...
  describe('calling next(): ', function() {
    ...
    it('exists the route if there is not more handler', function(done) {
      request(app).get('/').expect(404).end(done);
    })
  });

  describe("error handling:", function() {
    it("exists the route if there's an error", function(done) {
      route.use('get', function(req, res, next) {
        next(new Error('boom!'));
      });
      route.use('get', function(req, res, next) {
  ...
  describe("verb matching", function() {
    ...
    it('matched GET to the get handler' ,function(done) {
      request(app).get('/').expect('got').end(done);
    });

    it('matched POST to get handler', function(done) {
  ...
  describe('match any verb', function() {
    ...
    it('matched GET to the all handler', function(done) {
      request(app).get('/').expect('all').end(done);
    });
    it('matched POST to the all handler', function(done) {
